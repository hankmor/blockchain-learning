...
async function main() {
    ...
    console.log(`🔗 查看合约: https://sepolia.basescan.org/address/${token.address}`);
    ...
    console.log(`\n🔗 相关链接:`);
    console.log(`   BaseScan: https://sepolia.basescan.org/address/${token.address}`);
    console.log(`   Base Sepolia Faucet: https://sepoliafaucet.com/`);
    
    console.log(`\n💡 下一步操作:`);
    console.log(`   1. 验证合约: npx hardhat verify --network baseSepolia ${token.address}`);
...
// 查看合约: https://sepolia.basescan.org/address/0x84f899c674db5ab1f8f190d2007ae4003e2dda75