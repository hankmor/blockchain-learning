...
//
// 简单的公开拍卖合约示例。
// 总体思路是每个人都可以在投标期内发送他们的出价。 出价已经包含了资金/以太币，来将投标人与他们的投标绑定。 如果最高出价提高了（被其他出
// 价者的出价超过），之前出价最高的出价者可以拿回她的钱。 在投标期结束后，受益人需要手动调用合约来接收他的钱 - 合约不能自己激活接收。
//
...
contract SimpleAuction {
    ...
    // 出价人地址到出价金额的映射，可以取回的之前的出价
    mapping(address => uint) pendingReturns;

    // 拍卖结束标记：拍卖结束后设为 true，将禁止所有的变更
    bool ended;
